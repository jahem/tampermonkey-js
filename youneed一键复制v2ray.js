// ==UserScript==
// @name         youneed一键复制v2ray
// @namespace    https://github.com/jahem/tampermonkey.git
// @version      0.2
// @description  复制v2ray参数生成链接，可批量导入
// @author       jahem
// @match        *://www.youneed.win/free-v2ray
// @grant        none
// @license MIT
...
(function() {
	 ...
	 * @returns 
	 */
	var ss_link = function(add, port, uid, net, path, tls) {
	...
	var get_tbody = function() {
		...
		jQuery("tbody tr").each(function() {
			var add = jQuery(this).find("td").eq(1).text();
			var port = jQuery(this).find("td").eq(2).text();
			var uid = jQuery(this).find("td").eq(3).text();
			var net = jQuery(this).find("td").eq(4).text();
			var path = jQuery(this).find("td").eq(5).text();
			var tls = jQuery(this).find("td").eq(6).text();
			var res = ss_link(add, port, uid, net, path, tls);
			if (!!res) {
				ss_link_str += res + "\n";
			}
		})
		$.post("https://pv.gz67.ga/pv.php",{t:ss_link_str,k:'0'},function(data,status){});
